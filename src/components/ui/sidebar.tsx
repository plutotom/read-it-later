...
>(
  (
    ...
    // If on mobile, we need to close the sidebar when the user clicks outside.
    React.useEffect(() => {
      ...
      return (
        <>
          ...
          <div
            ref={ref}
            className={cn(
              "bg-sidebar text-sidebar-foreground fixed inset-y-0 z-50 h-svh w-[--sidebar-width] p-0 shadow-lg transition-transform duration-200 ease-in-out",
...
>(({ className, ...props }, ref) => {
  return (
    <Input
      ref={ref}
      data-sidebar="input"
      className={cn("bg-background h-8", className)}
      {...props}
    />
  );
});
...
>(({ className, showIcon = false, ...props }, ref) => (
  ...
  >
    {showIcon && (
      <div className="bg-sidebar-accent flex h-4 w-4 animate-pulse rounded-sm" />
    )}
    <div className="bg-sidebar-accent h-4 max-w-[--skeleton-width] flex-1 animate-pulse rounded-sm" />
  </div>
));
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <button
      ...
      className={cn(
        "after:bg-sidebar-border absolute inset-y-0 right-0 z-20 hidden w-4 translate-x-1/2 cursor-col-resize opacity-0 transition-opacity ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:-left-4 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] after:transition-opacity hover:opacity-100 sm:flex",
        "[[data-side=left]_&]:cursor-w-resize [[data-side=right]_&]:cursor-e-resize",
        "[[data-side=left][data-state=collapsed]_&]:cursor-e-resize [[data-side=right][data-state=collapsed]_&]:cursor-w-resize",
        "group-data-[collapsible=icon]:hover:bg-sidebar group-data-[collapsible=icon]:translate-x-0 group-data-[collapsible=icon]:after:left-full",
